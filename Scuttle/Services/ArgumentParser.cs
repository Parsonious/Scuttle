...
public class ArgumentParser
{
    ...
    public CliOptions Parse(string[] args)
    {
        ...
        for ( int i = 0; i < args.Length; i++ )
        {
            switch ( args[i].ToLower() )
            {
                ...
                case "--file":
                case "--input-file":
                    options.InputFile = GetNextArg(args, ref i);
                    break;
                case "--save-key":
                    options.SaveKeyToFile = true;
                    break;
                case "--key-file":
                    options.KeyFile = GetNextArg(args, ref i);
                    break;
                case "--silent":
                    options.Silent = true;
                    break;
    ...
    private void ApplyEnvironmentVariables(CliOptions options)
    {
        ...
        options.InputFile ??= Environment.GetEnvironmentVariable("TOKEN_INPUT_FILE");
        options.KeyFile ??= Environment.GetEnvironmentVariable("TOKEN_KEY_FILE");
    }
